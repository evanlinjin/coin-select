...
fn bump_of(ancestors: &[(u64, u64)], rate: FeeRate, set: &[u32]) -> u64 {
    ...
    ancestor_shortfall(rate, (weight, fee))
}

/// The fee still owed so ancestors with summed `(weight, fee)` meet `rate`: `ceil(weight · rate) −
/// fee`, saturating at 0.
///
/// Computed in `f64`, not with [`FeeRate::implied_fee_wu`]'s `f32`. The rate is an `f32`, so it has
/// a 24-bit significand and converts to `f64` exactly, and `weight · rate` is then exact for any
/// weight below 2^29 WU (about 537 million) — orders of magnitude beyond any real ancestor package.
/// Being exact is what lets the ancestor lower bounds in [`CoinSelector`] be compared against this
/// without a rounding allowance.
pub(crate) fn ancestor_shortfall(rate: FeeRate, (weight, fee): (u64, u64)) -> u64 {
    let owed = weight as f64 * rate.spwu() as f64 - fee as f64;
    if owed <= 0.0 {
        return 0;
    }
    // Round up without `f64::ceil`, which `no_std` lacks. Truncating a positive float rounds down,
    // so add one whenever that dropped a fraction.
    let truncated = owed as u64;
    if (truncated as f64) < owed {
